...
**使用哈希法的经典题目**。**四个独立的数组，只要找到A[i] + B[j] + C[k] + D[l] = 0就可以，不用考虑有重复的四个元素相加等于0的情况**
...
Pop() -> push() -> getMaxValue()

单调队列 -> 维持单调递增单调递减

## 4. 347 前K个高频元素

大顶堆，小顶堆，直接用map 加排序方法
...
1. 满二叉树 2k-1
2. 完全二叉树：底层不一定满的，底部一定从左到右连续的
3. 二叉搜索树：左子树节点小于中间节点，右子树大雨中间节点
4. 平衡二叉搜索树：左子树和右子树的高度值不能相差1
